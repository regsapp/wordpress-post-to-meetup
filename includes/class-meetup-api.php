...
class Meetup
{
    ...
    {
        return $this->post('/:urlname/events', $parameters);
    }
    /**
     ...
     *
     * @code
     * $meetup->patch('/2/member/:id', array('id'=>10));
     * @endcode
     */
    ...
    protected function api($url, $parameters, $action=self::GET, $contentType = null)
    {
        ...
        if (!is_null($this->_response) && ($status < 200 || $status >= 300))
        {
            if ($this->_response->errors) {
                $error = '';
                foreach ($this->_response->errors as $error_record) {
                    $error .= $error_record->message . '. ';
                }
            }
            //tell them what went wrong or just relay the status
            elseif( isset($this->_response->error) && isset($this->_response->error_description) )
            {
